...
def get_tickers(category="linear", limit=200):
    ...
    try:
        ...
        st.error(f"Error get_tickers: {e}")
        return None

@st.cache_data(ttl=300)
def get_instruments_info(category="linear", limit=200):
    """Ambil informasi instrumen (termasuk rank/market cap) dari Bybit Indonesia"""
    try:
        response = requests.get(
        ...
        st.error(f"Error get_instruments_info: {e}")
        return None

def get_klines(symbol, interval="D", limit=7):
    """Ambil data candlestick historis untuk volume trend"""
    try:
        response = requests.get(
...
def get_volume_avg_7d(symbol):
    """Ambil rata-rata volume 7 hari dari Bybit Indonesia"""
...
def process_combined_data(tickers, instruments, currency, usd_to_idr):
    ...
    symbols = [t["symbol"] for t in tickers[:100]]
...
# =========================================================
# CHART: Price Chart dengan Plotly (opsional)
# =========================================================
st.divider()
st.subheader("📈 Price Chart - 7 Days")

# Ambil data kline untuk simbol yang dipilih
